...
def explain(line,filename,explain_type):
    input_tensor = str_to_tensor([line],args.input_length)
    # print("shape = ",input_tensor.shape)
    output_class,_ = model(input_tensor)
    probs = softmax(output_class)
    # print("probs shape = ",probs.shape)
    scores = torch.zeros([len(line)])

    if explain_type == 'char':
        i=0 #track the position in the string
        for letter in line:
            letter_tensor = copy.deepcopy(input_tensor)
            letter_tensor[i+1] = torch.zeros(1,len(vocabulary)) #darn tensors and indexing!!!!!
            # print(letter_tensor)
            output_class_letter,_ = model(letter_tensor)
            letterProbs = softmax(output_class_letter)
            # print("line probs shape = ",lineProbs.shape)
            scores[i] = torch.dist(probs,letterProbs,2)
            i += 1
    elif explain_type == 'word':
        word_markers = [i for i, x in enumerate(line) if x == " "]
        word_markers = [-1] + word_markers + [len(line)] #pretend there's a space before and after the line
        for i in range(len(word_markers)-1): #loop through each word by start
            start = word_markers[i]+1
            end = word_markers[i+1]
            letter_tensor = copy.deepcopy(input_tensor)
            for letter in range(start,end):
                letter_tensor[letter+1] = torch.zeros(1,len(vocabulary))
            # print(letter_tensor)
            output_class_letter,_ = model(letter_tensor)
            letterProbs = softmax(output_class_letter)
            # print("line probs shape = ",lineProbs.shape)
            for letter in range(start,end):
                scores[letter] = torch.dist(probs,letterProbs,2)
    else:
        pass 
